...
1. 无用户提示不主动改；要改须用户明确要求，或先征得许可
2. 用户说「写进 AGENTS.md」→ 先做归属判断（见下）；用户坚持后再写入
3. 日常按已注入的 `AGENTS.md` 执行其他任务 → **不加载本技能**
4. 仓库尚无 `AGENTS.md` → **不默认补全**；有稳定非可推断约定或用户要求时再建
...
| 末尾维护署名（新建或大幅改写时，见工作流）           | 逐条修订说明、变更历史            |
...
**末尾署名（新建或大幅改写时）：** 正文最后单独一行。写**正在编写的文件路径**（根 `AGENTS.md`、`CLAUDE.md`，或嵌套如 `packages/api/AGENTS.md`），不写技能名；维护提醒指向加载 `using-agentsmd`。措辞随仓库语言、文件类型调整。
...
- 嵌套文件 → 署名用**该嵌套文件的路径**，不写根 `AGENTS.md`
- 编写 `CLAUDE.md` → 署名中用 `CLAUDE.md`
- 英文仓库可改为英文一句，语义相同即可
- 小改、署名仍准确：不必动；缺署名的大改：补上
